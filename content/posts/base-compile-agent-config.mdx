...
The honesty goes one level deeper: the Claude Code hook runs the `base` binary from PATH, and Claude Code treats a missing hook command as non-blocking. So `check` probes PATH and downgrades its own report to _assisted_ with a warning when the binary doesn't resolve. The tool audits its own enforcement claims.
