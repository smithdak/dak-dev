...
The eval design is the part worth stealing. The runner builds the skill listing exactly as the runtime presents it — every non-draft skill's name and description. Then it asks a judge model, per eval case, which single skill would fire for this user message. A should-trigger case passes only when the judge picks the skill under test. A should-not-trigger case passes when it picks anything else, _including a different skill_ — so confusion with a sibling counts against you. That makes the score a measure of selectivity against the whole catalog, not of the skill in isolation.
