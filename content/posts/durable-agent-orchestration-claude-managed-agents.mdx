...
The fix is to detect a _true_ finish: the session reports idle _after_ doing work (its stop reason is not `requires_action`), or it reports terminated. I poll newest-event-first and gate on both signals:
...
Downloading the artifact has its own trap. The Files API indexes a session's outputs one to three seconds _after_ the session goes idle, so an immediate listing comes back empty. The download needs a short retry loop, and every Files call needs **both** beta headers or the listing fails intermittently:
...
- Detect a Managed Agent's _true_ completion (idle-after-work or terminated), because sessions start idle and a naive check downloads an empty artifact
